...
class Historico:
    ...
    def gerar_relatorio(self, tipo_transacao=None):
        ...
        for transacao in self._transacoes:
            if (tipo := transacao['tipo']) == tipo_transacao or tipo_transacao == None:
                print(
                    str(tipo).center(20),
                    str(transacao['valor']).center(20),
...
class ContaCorrente(Conta):
    ...
    def __repr__(self) -> str:
        return f'<C.C. -> Ag.: {self.agencia} N.: {self.numero}>'

class ContaIterador:
    def __init__(self, conta:ContaCorrente) -> None:
        self._conta = conta
        self._inf = {
    ...
    def __next__(self):
        if self._cont < len(self._inf):
            keys = list(self._inf.keys())
            key = keys[self._cont]
            self._cont += 1
            return key + ' : ' + str(self._inf[key])
        raise StopIteration
...
def log_transacao(func):
    ...
    def wrap(*args, **kwargs):
        with open(LOG_FILE_DIR, 'a') as file:
            ...
            text = f'[{time}] : {func.__name__.upper()} Iniciado...\n'
            file.write(text)
            text = f'[{time}] : {func.__name__.upper()} : Args : {args} : KArgs : {kwargs} ;\n'
            file.write(text)

            try:
                return_func = func(*args, **kwargs)
                text = f'[{time}] : {func.__name__.upper()} : Retunr : {return_func} ;\n'
                file.write(text)
            except:
                text = f'[{time}] : ERRO NA EXECUCAO!!! ;\n'
                file.write(text)
...
@log_transacao
def retornar_conta(contas_correntes):
    conta_index = escolher_da_lista(contas_correntes)
    return contas_correntes[conta_index]


@log_transacao
def deposito(contas_correntes):
...
@log_transacao
def cadastro_pessoa_fisica(pessoas):
   ...
   #cadastrando usuarios para testes
    ...
    while True:
        ...
        if opcao == '1':
            ...
            sacar(contas_correntes)
            
        elif opcao == '3':
            extrato(contas_correntes)

        elif opcao == '4':
            criar_conta_corrente(pessoas, contas_correntes)
            
        elif opcao == '5':
            cadastro_pessoa_fisica(pessoas)

        elif opcao == '6':
            listar_contas(contas_correntes)

        elif opcao == '7':
            listar_usuarios(pessoas)
